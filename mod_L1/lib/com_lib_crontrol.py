...
def run_command(args_dict, threads, log_path, sat_pair, job_id, date_s, date_e):
    ...
    for key_name in sorted(args_dict.keys(), reverse=True):
        ...
        for cmd in args_dict[key_name]:
            pool.apply_async(
                command, (cmd, log_path, sat_pair, job_id, key_name))

    pool.close()
    pool.join()


def command(cmd, log_path, sat_pair, job_id, key_name):
    '''
    args_cmd: python a.py 20180101  (完整的执行参数)
    '''
    print cmd
    log_file = os.path.join(log_path, '%s.log' % sat_pair)
    time_start = datetime.now()
    ...
    if not os.path.isfile(log_file):
        ...
        fd = open(log_file, 'a+')
    line = time_start.strftime('%Y-%m-%d %H:%M:%S') + \
        ' %s %s %s times: %ds' % (sat_pair, job_id, key_name, all_time) + '\n'
    fd.write(line)
    fd.flush()
    fd.close()
