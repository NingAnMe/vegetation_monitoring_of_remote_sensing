...
def do_thumb_job(in_file):
    ...
    thumb_sql_cmd = 'mysql -h"10.135.30.112" -P"3306" -u"hzqx" -p"PassWord@1234" "hangZhou" -e "update m_product_data set thumb_path=\'%s\' where datetime=\'%s\' and data_type=\'%s\' and product_type=\'%s\' "' % (out_file,dateTime,data_type,product_type) 
    print cmd
    print push_cmd
    print thumb_sql_cmd
    os.system(cmd)
...
def push_hdffile_indb(sat_pair, ymd):
    ...
    for in_file in file_list:
        ...
        if '2000M' in filename or '4000M' in filename:
            push_cmd = '/usr/local/bin/wput -q --skip-larger --basename=%s %s ftp://HZQX:HZQX@10.135.30.112:21%s' % (dirname, in_file, dirname)
            select_cmd = 'select id from m_product_data where hdf_time=\'%s\' and hdf_type=\'%s\'' % (dateTime,data_type)
